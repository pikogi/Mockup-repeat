...
function StampsGrid({ stampsRequired, currentStamps, stampImageUrl, backgroundColor }) {
  ...
  const imageUrl = React.useMemo(() => {
    ...
    if (
      stampImageUrl.startsWith('http://') ||
      stampImageUrl.startsWith('https://') ||
      stampImageUrl.startsWith('data:')
    )
      ...
      >
        ...
        {showIcon && (
          <svg
            ...
            stroke="currentColor"
            viewBox="0 0 24 24"
            style={{ color: backgroundColor, width: `${iconSize}px`, height: `${iconSize}px` }}
          >
        ...
        {!isCompleted && !showImageInPreview && !isFirst && (
          <div
            className="rounded-full bg-white bg-opacity-50 border border-white border-opacity-50"
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <div className="flex items-baseline gap-1">
              ...
              <span className="text-2xl font-semibold opacity-80" style={{ color: foregroundColor }}>
                %
              </span>
            </div>
          </div>
...
function DiscountPreview({ card, foregroundColor, backgroundColor }) {
  ...
  return (
    <div className="relative h-44 overflow-hidden">
      ...
      <div
        className="absolute left-0 top-1/2 w-4 h-8 -translate-x-1/2 -translate-y-1/2 rounded-full"
        style={{ backgroundColor: '#f8fafc' }}
      />
      <div
        className="absolute right-0 top-1/2 w-4 h-8 translate-x-1/2 -translate-y-1/2 rounded-full"
        style={{ backgroundColor: '#f8fafc' }}
      />

      {/* Línea punteada decorativa */}
      <div
        className="absolute left-6 right-6 top-1/2 border-t-2 border-dashed opacity-20"
      ...
      <div className="relative h-full flex flex-col items-center justify-center p-4">
        ...
        <div className="text-center mb-2">
          <div className="flex items-center justify-center gap-1">
            ...
            <div className="flex flex-col items-start">
              <span className="text-2xl font-bold leading-none" style={{ color: foregroundColor }}>
                %
              </span>
              <span className="text-xs uppercase tracking-wider opacity-70" style={{ color: foregroundColor }}>
                OFF
              </span>
            </div>
          </div>
        ...
        {/* Secondary Fields */}
        <div className="px-4 py-1.5 rounded-full mt-2" style={{ backgroundColor: `${foregroundColor}15` }}>
      ...
      <div className="px-4 -mt-8">
        <div className="bg-white rounded-xl p-4 shadow-lg">
          <div className="flex items-center justify-between">
            ...
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor }}>
...
function AndroidCashbackPreview({ card, backgroundColor }) {
  ...
  return (
    <div className="relative h-48">
      {/* Hero con porcentaje */}
      <div className="h-24 w-full flex items-center justify-center" style={{ backgroundColor }}>
        ...
        <div className="bg-white rounded-xl p-3 shadow-lg">
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center p-2 rounded-lg" style={{ backgroundColor: `${backgroundColor}10` }}>
              ...
              <p className="text-xl font-bold" style={{ color: backgroundColor }}>
                ${accumulatedCashback}
              </p>
            </div>
...
export default function ProgramPreviewComponent({
  ...
  const foregroundColor =
    card.wallet_design?.hex_foreground_color || card.foreground_color || getTextColor(cardBackgroundColor)
  ...
  const renderAndroidProgramContent = () => {
    switch (programType) {
      case 'giftCard':
        return (
          <AndroidGiftCardPreview card={card} foregroundColor={foregroundColor} backgroundColor={cardBackgroundColor} />
        )
      case 'cashback':
        return (
          <AndroidCashbackPreview card={card} foregroundColor={foregroundColor} backgroundColor={cardBackgroundColor} />
        )
      case 'discount':
        return (
          <AndroidDiscountPreview card={card} foregroundColor={foregroundColor} backgroundColor={cardBackgroundColor} />
        )
      case 'membership':
        return (
          <AndroidMembershipPreview
            card={card}
  ...
  const CardBack = () => (
    ...
    >
      <div
        className="p-3 flex items-center justify-between gap-3 border-b flex-shrink-0"
        style={{ backgroundColor: cardBackgroundColor, borderColor: foregroundColor + '20' }}
      >
          ...
          {card.logo_url ? (
            <div className="h-9 max-w-[108px] rounded-lg overflow-hidden flex-shrink-0 flex items-center">
              <img
                key={card.logo_url}
                src={
                  card.logo_url?.startsWith('http') || card.logo_url?.startsWith('data:')
                    ? card.logo_url
                    : `${import.meta.env.VITE_API_URL?.replace('/api', '') || 'http://localhost:3000'}${card.logo_url}`
                }
        ...
        <p className="text-base flex-shrink-0" style={{ color: labelColor }}>
          {card.club_name || ''}
        </p>
      </div>
      ...
      <div className="p-3 flex-1 overflow-y-auto" style={{ minHeight: 0, maxHeight: '100%' }}>
        <div className="space-y-3">
          {card.terms && (
            <div>
              <p className="text-xs uppercase mb-2 opacity-70 font-semibold" style={{ color: foregroundColor }}>
                Términos y Condiciones
              </p>
              <p className="text-xs leading-relaxed break-words" style={{ color: foregroundColor }}>
                {card.terms}
              </p>
            </div>
          )}
          {(card.contact_email || card.contact_phone || card.website) && (
            <div className="mt-1">
              {card.contact_email && (
                <>
                  <div
                    style={{ borderTopColor: foregroundColor + '25', borderTopWidth: 1 }}
                    className="flex items-center justify-between py-2.5"
                  >
                    <span className="text-xs font-medium" style={{ color: foregroundColor }}>
                      Correo electrónico
                    </span>
                    <span className="text-xs font-medium text-blue-400 truncate ml-2 max-w-[55%] text-right">
                      {card.contact_email}
                    </span>
                  </div>
                </>
      ...
      <div
        className="py-3 border-t text-center flex-shrink-0"
        style={{ backgroundColor: cardBackgroundColor, borderColor: foregroundColor + '20' }}
      >
        <p className="text-xs opacity-70" style={{ color: foregroundColor }}>
          by Repeat.la
        </p>
      </div>
    </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="flex-shrink-0">{renderProgramContent()}</div>
          ...
          >
            ...
            <p className="text-xs mt-1.5 opacity-70" style={{ color: foregroundColor }}>
              by Repeat.la
            </p>
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col p-4 gap-3 flex-shrink-0" style={{ backgroundColor: cardBackgroundColor }}>
            ...
            {/* Fila 2: Club name */}
            <h2 className="text-white text-2xl font-bold leading-tight">{card.club_name || card.program_name || ''}</h2>
            ...
            <div className="flex justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-white text-sm font-semibold">
                  {currentStamps}/{stampsRequired} - 0
                </p>
              </div>
            </div>
        ...
        >
          <div className="flex-1 overflow-y-auto">
            <div className="p-5 space-y-5">
              ...
              {card.club_name && <p className="text-base font-bold text-white">{card.club_name}</p>}
              {/* Términos y Condiciones */}
              {card.terms && (
                <div>
      ...
      >
        ...
        >
          ...
          >
            ...
            <div className="flex-shrink-0 bg-white">{renderProgramContent()}</div>
