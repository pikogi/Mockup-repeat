...
import { User, LogOut, Crown, Mail, Globe, Lock, Store, Loader2, Sun, Moon, Monitor } from 'lucide-react'
...
export default function Profile() {
  ...
  const userBranch = assignedBranchId ? stores.find((s) => (s.store_id || s.id) === assignedBranchId) : stores[0]
        ...
        <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900 dark:text-gray-100 truncate">
                    {userBranch?.store_name || userBranch?.name || t('unassigned')}
                  </p>
                </div>
              </div>
              ...
              {import.meta.env.DEV && (
                <div className="mt-6 bg-gray-900 text-white rounded-xl p-4 text-xs font-mono overflow-auto max-h-96">
                  ...
                  <div className="space-y-1">
                    <div>
                      <span className="text-gray-400">type_user:</span>{' '}
                      <span className="text-green-400">{user?.type_user || 'null'}</span>
                    </div>
                    <div>
                      <span className="text-gray-400">user_type:</span>{' '}
                      <span className="text-green-400">{user?.user_type || 'null'}</span>
                    </div>
                    <div>
                      <span className="text-gray-400">user_id:</span>{' '}
                      <span className="text-blue-400">{user?.user_id || 'null'}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      <span className="text-gray-400">brand_id:</span>{' '}
                      <span className="text-purple-400">{user?.brand_id || 'null'}</span>
                    </div>
                    <div>
                      <span className="text-gray-400">branch_id:</span>{' '}
                      <span className="text-purple-400">{user?.branch_id || 'null'}</span>
                    </div>
                    <div className="mt-4 pt-4 border-t border-gray-700">
                      <div className="text-yellow-400 font-bold mb-2">JWT completo (desde localStorage):</div>
                      <div className="space-y-1 mb-2">
                        <div>
                          <span className="text-gray-400">user_type (del JWT):</span>{' '}
                          <span className="text-green-400">{user?.user_type || 'null'}</span>
                        </div>
                      </div>
                      <details>
                        <summary className="cursor-pointer text-blue-400 hover:text-blue-300">
                          Ver contenido completo del token
                        </summary>
                        <pre className="mt-2 p-2 bg-black rounded overflow-auto">{JSON.stringify(user, null, 2)}</pre>
                      </details>
                    </div>
                  </div>
                </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <Card className="p-6 border-0 shadow-lg space-y-6">
              ...
              {/* Language Selector */}
              <div className="bg-blue-50/50 dark:bg-blue-950/30 p-4 rounded-xl">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">
                  {t('language')}
                </label>
                <Select value={language} onValueChange={changeLanguage}>
                  <SelectTrigger>
                    <Globe className="w-4 h-4 mr-2 text-blue-500 dark:text-blue-400" />
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center gap-1.5 rounded-lg p-3 text-sm font-medium transition-colors ${
                        theme === value
              ...
              {/* Password Change */}
              <div className="bg-amber-50/50 dark:bg-amber-950/30 p-4 rounded-xl">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">
                  {t('password')}
                </label>
                <Button
                ...
                >
                  {resetPasswordMutation.isPending ? (
                    ...
                    <>
                      <Lock className="w-4 h-4 mr-2 text-amber-500 dark:text-amber-400" />
                      {t('changePassword')}
                    </>
                  )}
                </Button>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <Card className="p-6 border-0 shadow-lg">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t('subscription')}</h3>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            <Button
              variant="outline"
              className="w-full h-12 border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950"
