...
export default function ScanQR() {
  ...
  const handleScan = async (cardId) => {
    ...
    try {
      ...
      setStep('error');
      const authFailed = err?.response?.status === 401 || err?.response?.status === 403
      ...
      <div className="fixed inset-0 bg-black z-50 flex items-center justify-center p-4">
        <Card className="p-8 w-full max-w-md bg-white">
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-gray-900">{t('selectLocation')}</h2>
            <p className="text-gray-600">{t('whereWorking')}</p>
          </div>
  ...
  if (loadingStores) {
    return (
      <div className="fixed inset-0 bg-black z-50 flex items-center justify-center p-4">
        <Card className="p-8 w-full max-w-md bg-white text-center">
    ...
    return (
      <div className="fixed inset-0 bg-black z-50 flex items-center justify-center p-4">
        <Card className="p-8 w-full max-w-md bg-white text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">Sin tiendas configuradas</h2>
          <p className="text-gray-600 mb-6">Necesitas tener al menos una tienda configurada para escanear códigos QR.</p>
    ...
    <div className="fixed inset-0 bg-black z-50">
      ...
      <div className="absolute top-4 left-4 z-10 bg-black/50 backdrop-blur-md rounded-full px-4 py-2 flex items-center gap-2 text-white border border-white/20">
        ...
        <span className="text-sm font-medium">
          {stores.find(s => (s.store_id || s.id) === selectedStore)?.store_name ||
           stores.find(s => (s.store_id || s.id) === selectedStore)?.name ||
           t('loading')}
        </span>
          ...
          {hasCamera ? (
            ...
            <div className="flex items-center justify-center h-full">
              <Card className="p-8 text-center mx-4">
                ...
                <p className="text-gray-500 mb-4">{t('allowCameraAccess')}</p>
            ...
            <Card className="p-8 text-center w-full">
              ...
              {step === 'scan' && processing && (
                <div className="flex flex-col items-center justify-center py-4">
                  ...
                  <p className="text-gray-600">Leyendo tarjeta...</p>
                </div>
              )}
              ...
              {step === 'review' && cardData && (() => {
                ...
                return (
                  <>
                    ...
                    {card.customer?.email && (
                      <p className="text-sm text-gray-400 mb-1">{card.customer.email}</p>
                    )}
                    {card.created_at && (
                      ...
                      return (
                        <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-xl text-left">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-sm font-semibold text-green-800">Premio listo para canjear</span>
                          </div>
                          <p className="text-xs text-green-700 mb-2">{card.program?.reward_description}</p>
                          <Button
                            size="sm"
              ...
              {step === 'error' && (
                <>
                  ...
                  <p className="text-gray-600 mb-6">{errorMsg || "No se pudo procesar"}</p>
                  <div className="flex gap-3">
                    {isAuthError ? (
                      <>
                        <Button variant="outline" className="flex-1" onClick={handleClose}>
