...
export default function Stores() {
  ...
  const getQrUrl = () => {
    ...
    const activeIds = programs
      .filter((p) => p.is_active !== false && (p.store_ids?.length === 0 || p.store_ids?.includes(storeId)))
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 md:py-12">
        ...
        {isLoading ? (
          <Card className="p-12 text-center bg-gray-50 dark:bg-gray-800 border-dashed">
            <Store className="w-12 h-12 text-gray-300 dark:text-gray-600 mx-auto mb-4 animate-pulse" />
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Cargando sucursales...</h3>
          </Card>
        ) : stores.length === 0 ? (
          <Card className="p-12 text-center bg-gray-50 dark:bg-gray-800 border-dashed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {stores.map((store) => (
              <motion.div key={store.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
                    ...
                    <div className="flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            <AlertDialogCancel disabled={isDeletingStore(store.id)}>Cancelar</AlertDialogCancel>
                            <AlertDialogAction
                              onClick={() => {
                ...
                <>
                  <div className="bg-white dark:bg-gray-900 p-4 rounded-xl shadow-lg border border-gray-100 dark:border-gray-800">
                    <img
                      ...
                      alt={`Código QR de ${selectedQrStore.store_name || selectedQrStore.name}`}
                      className="w-48 h-48"
                      loading="lazy"
                    />
                  </div>
                  <div className="text-center text-sm text-gray-500 dark:text-gray-400">
                    <p className="font-medium text-gray-900 dark:text-gray-100">
                      {selectedQrStore.store_name || selectedQrStore.name}
                    </p>
                    {getQrUrl() ? (
