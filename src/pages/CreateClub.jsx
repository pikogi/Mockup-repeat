...
export default function CreateClub() {
  ...
  // Pre-cargar logo de la brand en modo creación
  useEffect(() => {
    if (editId) return
    const logoUrl = brandData?.logo_url || storePrograms[0]?.brand?.logo_url || null
    if (logoUrl) {
      setFormData((prev) => ({ ...prev, logo_url: prev.logo_url || logoUrl }))
    }
  }, [brandData, storePrograms[0]?.brand?.logo_url]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    if (existingProgram && !dataLoaded) {
      ...
      const programFromStore = storePrograms.find((p) => (p.program_id || p.id) === existingProgram.program_id)
      ...
      const savedImages = (() => {
        try {
          return JSON.parse(localStorage.getItem(`program_images_${existingProgram.program_id}`) || '{}')
        } catch {
          return {}
        }
      })()
      ...
      setFormData({
        ...
        logo_url:
          ...
          programFromStore?.brand?.logo_url ||
      ...
      initialFormData.current = {
        ...
        label_color: walletDesign.hex_label_color || '#FFFFFF',
        terms:
          existingProgram.metadata?.terms_and_conditions ||
          existingProgram.program_rules?.terms_and_conditions ||
          existingProgram.program_rules?.terms ||
          '',
  ...
  useEffect(() => {
    ...
    setFormData((prev) => ({
      ...
      background_image_url: newUpload.background
        ...
        : images.stamp_background || savedImages.background || prev.background_image_url,
      stamp_image_url: newUpload.stamp
        ? prev.stamp_image_url
        : images.stamp_icon || savedImages.stamp || prev.stamp_image_url,
    }))
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('[CreateClub] handleSubmit images', {
      background: formData.background_image_url
        ? {
            ...
            preview:
              typeof formData.background_image_url === 'string' ? formData.background_image_url.substring(0, 50) : null,
          }
        : null,
      stamp: formData.stamp_image_url
        ? {
            ...
            preview: typeof formData.stamp_image_url === 'string' ? formData.stamp_image_url.substring(0, 50) : null,
          }
        : null,
    })
    ...
    if (editId) {
      ...
      if (result.success) {
        ...
        const prevImages = (() => {
          try {
            return JSON.parse(localStorage.getItem(`program_images_${idToUpdate}`) || '{}')
          } catch {
            return {}
          }
        })()
            ...
            <Card className="p-8 shadow-xl border-0">
              <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex gap-3 items-center">
                    ...
                    <label className="cursor-pointer">
                      ...
                      <div
                        ...
                        onClick={() => setIsFlipped(false)}
                      >
                        {uploadingStamp ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4" />}
                  ...
                  <div className="grid gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, security_cooldown_hours: parseInt(e.target.value) || 0 })
                        }
                        placeholder="0"
                        className="h-12"
                      />
                ...
                <Button
                  ...
                  disabled={isCreating || isUpdating || uploadingLogo || uploadingBackground || uploadingStamp}
                >
