...
export default function ForgotPassword() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (!email.trim()) {
      toast.error(t('forgotEmailRequired'))
      return
    }
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md">
        <Card className="bg-white shadow-xl">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl font-bold text-gray-900">{t('forgotTitle')}</CardTitle>
            <CardDescription className="text-base pt-2">{t('forgotDesc')}</CardDescription>
          </CardHeader>
