...
import { CreditCard, Users, QrCode, Gift, Calendar as CalendarIcon, Store } from 'lucide-react'
...
export default function DashboardHome({ brandId }) {
  ...
  const { data: totalMembersCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      if (!brandId || programIds.length === 0) return 0
      ...
      const results = await Promise.all(programIds.map((pid) => fetchAllPages({ programId: pid }).catch(() => [])))

      const unique = new Set(
        results
          .flat()
          .map((e) => e.email || e.user_id || e.id)
          .filter(Boolean),
      )
      return unique.size
    },
    enabled: !!brandId && programIds.length > 0,
    staleTime: 5 * 60 * 1000,
  })
    ...
    queryFn: async () => {
      ...
      if (programIds.length > 0) {
        const results = await Promise.all(
          programIds.map((pid) =>
            api.brands
              .getUsers(brandId, { programId: pid, from, to })
  ...
  }, [rangedUsers, brandStats, dateFilter, customDateKey]) // eslint-disable-line react-hooks/exhaustive-deps
