...
const PROGRAM_IMAGES_BASE_URL = `https://${S3_BUCKET}.s3.us-east-1.amazonaws.com`
...
class ApiClient {
  ...
  auth = {
    login: async (email, password) => {
      ...
      if (token) {
        ...
        const userType =
          decodedToken?.user_type || decodedToken?.userType || decodedToken?.type_user || decodedToken?.typeUser
  ...
  brands = {
    ...
    getStatsUser: (brandId, userId) => this.get(`/brands/${brandId}/stats/users/${userId}`),
  ...
  stores = {
    ...
    update: (storeId, data) => this.patch(`/stores/${storeId}`, data),

    delete: (storeId) => this.delete(`/stores/${storeId}`),
  }
  ...
  loyaltyPrograms = {
    ...
    update: (programId, data) => this.patch(`/loyalty-programs/${programId}`, data),
  ...
  // ---------- REDEMPTIONS ----------
  redemptions = {
    update: (redemptionId, status) => this.patch(`/redemptions/${redemptionId}`, { status }),
  }
